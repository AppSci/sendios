...
func TestSendEmailWithError(t *testing.T) {
	require.NoError(t, godotenv.Load())
	...
	require.Error(t, err)
	require.Equal(t, "Request validation error:  data - This value should contain key `user`", err.Error())
	require.Equal(t, "ERROR", result.Meta.Status)
}
