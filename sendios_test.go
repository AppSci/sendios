...
func TestMappingError(t *testing.T) {
	...
	require.NoError(t, mapResponse([]byte(data), &resp))
...
func TestLoadingFromEnv(t *testing.T) {
	require.NoError(t, os.Setenv("SENDIOS_CONFIG", "client_id=111&client_token=xxx&project=333"))

	c, err := NewFromEnv()
	require.NoError(t, err)
	require.NotNil(t, c)
	assert.Equal(t, 111, c.Config.ClientID)
	assert.Equal(t, "xxx", c.Config.ClientToken)
	assert.Equal(t, 333, c.Config.Project)
}
